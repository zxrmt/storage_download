...
# Check if page_token.txt exists
try:
    f = open("last_page_token.txt", "r")
    pageToken = f.read()
    f.close()
except:
    print("No last_page_token.txt file")
...
while True:
    ...
    for data in res['items']:   
        ...
        cursor.execute('''
            INSERT INTO files (kind, mediaLink, name, size, updated, cr_commit_position_number, cr_git_commit, cr_commit_position)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            data['kind'],
            data['mediaLink'],
    ...
    # Write last page token to file
    f = open("last_page_token.txt", "w")
    f.write(pageToken)
    f.close()
